...
Antes de fazer isso, você deve estar familiarizado com os conceitos em [componentes nativos da interface do usuário](native-components-ios). Você também deve se familiarizar com o [código nativo para visualizações da web](https://github.com/react-native-webview-bootpay/react-native-webview-bootpay/blob/master/apple/BPCWebViewManager.m), pois você terá para usar isso como referência ao implementar novos recursos, embora não seja necessário um entendimento profundo.
...
```
[BPCWebView setClientAuthenticationCredential:credential];
```
...
-(void)installCerts {
  ...
  [BPCWebView setCustomCertificatesForHost:certMap];
}
...
Se você não tiver certeza de como algo deve ser implementado do lado do JS, consulte [WebView.ios.tsx](https://github.com/react-native-webview-bootpay/react-native-webview-bootpay/blob/master/src/WebView.ios.tsx) na fonte React Native WebView.
