...
1) Rodando os testes do react-native-webview-bootpay
2) Testando em um projeto `react-native init`

### Rodando os testes dentro do react-native-webview-bootpay
...
Ao fazer uma alteração, você provavelmente precisará desvincular, remover, adicionar novamente e vincular novamente o `react-native-webview-bootpay`:

```
$ react-native unlink react-native-webview-bootpay && yarn remove react-native-webview-bootpay
$ yarn add ../react-native-webview-bootpay && react-native link react-native-webview-bootpay
```
